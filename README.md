...
- **World tab** — a DM worldbuilding hub:
  ...
  - **Shops** — merchant inventories with items, prices, and stock; players buy items straight into their character's inventory (currency auto-deducted, stock decremented). The DM can **present a shop as a live pop-up storefront** to all players or a chosen subset — targeted players get a modal they buy from in real time (prices, stock, and their own currency all enforced), and the DM can stop showing it at any time.
