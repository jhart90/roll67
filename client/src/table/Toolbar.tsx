...
function EditPill({ macro, index, total }: { macro: Macro; index: number; total: number }) {
  ...
  return (
    <div className="pill-editor">
      ...
      <div className="pill-edit-actions">
        ...
        <button className="link danger" onClick={() => intents.deleteMacro(macro.id)}>delete</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="toolbar">
      {macros.map((m, i) => (
        <div key={m.id} className="pill-wrap">
          <button
            ...
            title={m.characterId ? 'Sheet roll' : m.command}
          >
            {m.name}
          </button>
          {editing && <EditPill macro={m} index={i} total={macros.length} />}
        </div>
      ))}
      ...
      {editing && (adding ? (
        ...
        <button className="pill-add" onClick={() => setAdding(true)}>+ pill</button>
      ))}

      <button className="toolbar-edit" title="Edit toolbar" onClick={() => { setEditing((v) => !v); setAdding(false); }}>
        {editing ? 'done' : '✎'}
      </button>
      {macros.length === 0 && !editing && (
        <span className="dim toolbar-hint">Pin rolls from a character sheet, or edit ✎ to add a pill</span>
      )}
    </div>
  );
}
