...
const SWADE_SHEET: Record<string, string> = {
  ...
  Handling: 'The vehicle’s responsiveness, added to or subtracted from every maneuvering roll its operator makes. Each Wound costs a point of it, to −4.',
  'Top Speed (mph)': 'Real-world miles per hour, used relatively — what matters in a chase is who is faster. Locomotion critical hits shave 10% of the base each.',
  Kind: 'What skill drives it: ground is Driving, watercraft Boating, aircraft and spacecraft Piloting.',
  ...
  'Linked (free to)': 'The attribute this skill hangs off, and the die it can be raised to for one build point per step. Past that ceiling every step costs two, so this is the number to check before spending points. A skill shown in amber has already outgrown its attribute.',
