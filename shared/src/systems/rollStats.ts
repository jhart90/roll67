...
export interface RollStatsSummary {
  /** Total individual dice ever rolled. */
  lifetime: number;
...
function parseKey(key: string): { n: number; sides: number } {
...
export function summarizeRollStats(rows: RollStatRow[]): RollStatsSummary {
  ...
  const maxPips = dieRows.reduce((a, r) => a + parseKey(r.key).sides * r.count, 0);
  return {
    lifetime,
    luck: maxPips > 0 ? (pips / maxPips) * 100 : 0,
    bySize: summarizeKeys(dieRows),
    byAmount: summarizeKeys(rows.filter((r) => r.kind === 'expr')),
  };
}
